...
function handleArrowLeftOutsideInline(event, change, next, opts) {
  ...
  if (isInlineBanned(upcomingNode, opts) || !hasStickyBoundaries || isExtending || upcomingNode.isVoid || textNodeIndex === -1) return next()
...
export default function onArrowLeft(event, change, next, opts) {
  ...
  if (change.value.focusInline) return handleArrowLeftInsideInline(event, change, next, opts)
