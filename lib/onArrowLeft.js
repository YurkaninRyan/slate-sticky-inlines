...
function handleArrowLeftOutsideInline(event, data, change, editor, opts) {
  ...
  return change.call(moveToEndOf, upcomingNode, data, event)
}
...
function handleArrowLeftInsideInline(event, data, change, editor, opts) {
  const isExtending = data.isShift
  const { hasStickyBoundaries } = opts
  ...
  if (change.state.focusInline.text === ZERO_WIDTH_SPACE && upcomingNode) {
    return change.call(moveToEndOf, upcomingNode, data, event, -1)
  }

  if (isAtSecondToFirstCharacter) {
    return change.call(moveToStartOf, change.state.focusInline, data, event)
  }
...
export default function onArrowLeft(event, data, change, editor, opts) {
  if (data.isCtrl) return null

  // In these cases we are actually inside the inline.
  if (change.state.focusInline) return handleArrowLeftInsideInline(event, data, change, editor, opts)

  return handleArrowLeftOutsideInline(event, data, change, editor, opts)
}
