...
export function moveToEndOf(change, node, data, event, offset = 0) {
  event.preventDefault()
  return data.isShift ? change.extendToEndOf(node).extend(offset) : change.collapseToEndOf(node).move(offset)
}
...
export function moveToStartOf(change, node, data, event, offset = 0) {
  event.preventDefault()
  return data.isShift ? change.extendToStartOf(node).extend(offset) : change.collapseToStartOf(node).move(offset)
}
