...
function StickyInlines(opts) {
  ...
  function onKeyDown(event, change, next) {
    ...
    if (
      ...
      isInlineBanned(change.value.focusInline, opts)
    )
      return next();
    ...
    switch (event.which) {
      case ARROW_LEFT:
        return onArrowLeft(event, change, next, opts);
      case ARROW_RIGHT:
        return onArrowRight(event, change, next, opts);
      case BACKSPACE:
        return onBackspace(event, change, next, opts);
      case DELETE:
        return onDelete(event, change, next, opts);
      default:
        return next();
    }
  }
  ...
  function onChange(change, next) {
    ...
    const toRemove = change.value.document
      ...
      .reduce((failures, inline) => {
        const hasFocus =
          change.value.isFocused && change.value.selection.hasEdgeIn(inline);
