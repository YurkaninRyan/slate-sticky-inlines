...
function handleArrowRightOutsideInline(event, change, next, opts) {
  ...
  if (isInlineBanned(upcomingNode, opts) || !hasStickyBoundaries || isExtending || upcomingNode.isVoid) return next()
...
function handleArrowRightInsideInline(event, change, next, opts) {
  const isExtending = event.shiftKey
  const { hasStickyBoundaries } = opts
...
export default function onArrowRight(event, change, next, opts) {
  if (event.ctrlKey) return next()

  // In these cases we are actually inside the inline.
  if (change.value.focusInline) return handleArrowRightInsideInline(event, change, next, opts)

  return handleArrowRightOutsideInline(event, change, next, opts)
}
